document.addEventListener("DOMContentLoaded", function () {
  ...
  function filtrarPokemons() {
    ...
    let filteredPokemons = pokemonsData.filter((pokemon) => {
      ...
      // Filtrar por tipo
      let matchTipo = tipo
        ? pokemon.types.some((t) => t.type.name.toLowerCase() === tipo)
        : true;
  ...
  document
    ...
    .addEventListener("click", filtrarPokemons);

  // Configurar el listener cuando pulso enter
  document
    .getElementById("pokemonName")
    .addEventListener("keypress", function (event) {
      if (event.key === "Enter") {
        event.preventDefault(); // Evitar que el formulario se envíe si es un form
        filtrarPokemons();
      }
    });
